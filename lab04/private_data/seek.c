...
static int mycdrv_open(struct inode *inode, struct file *file)
{
    ...
    my_ramdisk = (ramdisk)kmalloc(sizeof *my_ramdisk, GFP_KERNEL);
    my_ramdisk->ramdisk_size = PAGE_SIZE;
    my_ramdisk->data = (char*)kcalloc(my_ramdisk->ramdisk_size, sizeof *(my_ramdisk->data), GFP_KERNEL);
    file->private_data = my_ramdisk;
...
{
    ramdisk my_ramdisk = file->private_data;
    kfree(my_ramdisk->data);
    kfree(my_ramdisk);
...
mycdrv_read(struct file *file, char __user * buf, size_t lbuf, loff_t * ppos)
{
    ...
    ramdisk my_ramdisk = file->private_data;
	maxbytes = my_ramdisk->ramdisk_size - *ppos;
	bytes_to_do = maxbytes > lbuf ? lbuf : maxbytes;
	if (bytes_to_do == 0)
		printk(KERN_INFO "Reached end of the device on a read");
	nbytes = bytes_to_do - copy_to_user(buf, my_ramdisk->data + *ppos, bytes_to_do);
	*ppos += nbytes;
             ...
             loff_t * ppos)
{
	...
	nbytes =
	    bytes_to_do - copy_from_user(my_ramdisk->data + *ppos, buf, bytes_to_do);
	*ppos += nbytes;
...
static loff_t mycdrv_lseek(struct file *file, loff_t offset, int orig)
{
	...
	case SEEK_CUR:
        ...
        break;
	case SEEK_END:
        testpos = my_ramdisk->ramdisk_size + offset;
        break;
	default:
        testpos = -EINVAL;
        break;
	...
	if (testpos > -1 && testpos < my_ramdisk->ramdisk_size)
        file->f_pos = testpos;
	else
        testpos = -EINVAL;
